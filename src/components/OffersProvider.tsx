...
export function OffersProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    const fetchGlobalConfigs = async () => {
      try {
        ...
        if (res.ok) {
          ...
          console.error("Failed Local API Fetch Offers Config: ", e);
      } finally {
          setIsLoaded(true);
      }
    };
    ...
    // Aggressive Polling & Focus Fetching for True High-Speed Sync
    const isStorefront = typeof window !== 'undefined' && !window.location.pathname.startsWith('/admin');
    
    if (isStorefront) {
       // Regular background heartbeat
       const interval = setInterval(fetchGlobalConfigs, 3000); 
       
       // Instant pull the exact millisecond the user switches tabs back to Storefront 
       const handleFocus = () => fetchGlobalConfigs();
       window.addEventListener('focus', handleFocus);
       window.addEventListener('visibilitychange', () => {
         if (document.visibilityState === 'visible') fetchGlobalConfigs();
       });
