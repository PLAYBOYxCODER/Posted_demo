...
class InfiniteGridMenu {
  ...
  constructor(canvas: HTMLCanvasElement, items: any[], onActiveItemChange: Function, onMovementChange: Function, onInit: any = null, scale = 1.0) {
    this.canvas = canvas;
    this.items = items || [];
  ...
  init(onInit: any) {
    ...
    const gl = this.gl;
    if (!gl) {
      console.warn('No WebGL 2 context!');
      return;
    }
        ...
        item =>
          new Promise(resolve => {
            ...
            img.onerror = () => {
               ...
               fallback.src = 'https://picsum.photos/500?grayscale';
               fallback.onload = () => resolve(fallback);
               fallback.onerror = () => resolve(fallback); // Resolve anyway to prevent freezing
            };
            img.src = item.image || item.img || item.iconUrl || 'https://picsum.photos/500?grayscale';
          })
      )
    ).then((images: any) => {
      if(!ctx) return;
  ...
  onControlUpdate(deltaTime: number) {
    ...
    if (!this.control.isPointerDown) {
      ...
      if(nearestVertexIndex !== null && nearestVertexIndex !== undefined) {
         const itemIndex = nearestVertexIndex % Math.max(1, this.items.length);
         this.onActiveItemChange(itemIndex);
         const vertexPos = this.getVertexWorldPosition(nearestVertexIndex);
         const snapDirection = vec3.normalize(vec3.create(), vertexPos);
...
export default function InfiniteMenu({ items = [], scale = 1.0 }: { items: any[], scale?: number }) {
  ...
  return (
    <div style={{ position: 'relative', width: '100%', height: '100%' }}>
      ...
      {activeItem && (
        <>
          <h2 className={`face-title ${isMoving ? 'inactive' : 'active'}`}>{activeItem.title || activeItem.text}</h2>

          <p className={`face-description ${isMoving ? 'inactive' : 'active'}`}> {activeItem.description || "Discover more"}</p>
